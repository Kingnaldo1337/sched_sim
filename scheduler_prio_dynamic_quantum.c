...
struct proc * scheduler(struct proc * current)
{
    ...
    if (current != NULL)
    {
        ...
        {
            case READY:
                // Manter o processo na fila correta com base no uso do QUANTUM
            ...
            case BLOCKED:
                // Processos que saem de bloqueado voltam para `ready`
                current->queue = 0;
                enqueue(ready, current);
    ...
    // Estratégia PRIO_DYNAMIC_QUANTUM: escolher fila com probabilidade 80% (`ready`) e 20% (`ready2`)
